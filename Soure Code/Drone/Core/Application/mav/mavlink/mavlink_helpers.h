...
#endif
{
	...
	uint16_t temp_checksum = msg->checksum;
	crc_accumulate_buffer(&temp_checksum, _MAV_PAYLOAD(msg), msg->len);
	msg->checksum = temp_checksum;
#if MAVLINK_CRC_EXTRA
	//crc_accumulate(crc_extra, &msg->checksum);
	uint16_t temp_checksum_1 = msg->checksum;
	crc_accumulate(crc_extra, &temp_checksum_1);
	msg->checksum = temp_checksum_1;
#endif
...
MAVLINK_HELPER void mavlink_start_checksum(mavlink_message_t* msg)
{
	...
	uint16_t temp_checksum_2 = msg->checksum;
	crc_init(&temp_checksum_2);
	msg->checksum = temp_checksum_2;
}

MAVLINK_HELPER void mavlink_update_checksum(mavlink_message_t* msg, uint8_t c)
{
	//crc_accumulate(c, &msg->checksum);
	uint16_t temp_checksum_3 = msg->checksum;
	crc_accumulate(c, &temp_checksum_3);
	msg->checksum = temp_checksum_3;
}
